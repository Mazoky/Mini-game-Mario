...
#Изображения объектов и их размеры
...
#Стандартные переменные персонажей ;)
class Entity:
    ...
    def update(self):
        ...
        if self.is_dead:
           if self.rect.top > height - GROUND_height:
               self.is_out = True 
        else:
            self.handle_input()
...
#Класс игрока, отвечающий за его механику(движения)
class Player(Entity):
    def __init__(self):
    ...
    def respawn(self):
        self.is_out = False
        self.is_dead = False
        self.rect.midbottom = (width // 2, height - GROUND_height)
    ...
    for e in pygame.event.get():
        if e.type == pygame.QUIT:
            ...
            if player.is_out:
                score = 0
                spawn_delay = INIT_DELAY
                ladt_spawn_time = pygame.time.get_ticks()
                player.respawn()
                goombas.clear()
